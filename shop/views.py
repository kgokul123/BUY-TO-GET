...
def add_product_view(request):
    if request.method == 'POST':
        ...
        image_file = request.FILES.get('image') # உங்கள் HTML ஃபார்மில் உள்ள file input பெயர்

        if image_file:
            
            temp_path = os.path.join(settings.BASE_DIR, image_file.name)
            with open(temp_path, 'wb+') as destination:
                for chunk in image_file.chunks():
                    destination.write(chunk)
            
            
            drive_image_url = upload_image_to_drive(temp_path, image_file.name)
            ...
            Product.objects.create(
                name=product_name,
                price=product_price,
                image_url=drive_image_url  # கூகுள் டிரைவ் லிங்க் இங்கே சேவ் ஆகும்!
            )
            
            return redirect('success_url') # வெற்றி பெற்றதும் செல்லும் பக்கம்

    return render(request, 'your_template.html')

# கூகுள் டிரைவ் அப்லோட் பங்க்ஷன்
def upload_image_to_drive(file_path, file_name):
    # credentials.json ஃபைல் பாதை
    ...
    # நீங்கள் கூகுள் டிரைவில் உருவாக்கிய ஃபோல்டரின் ID-யை இங்கே போட வேண்டும் (கீழே எப்படி எடுப்பது எனப்பட்டுள்ளது)
    ...
    # கூகுள் டிரைவில் ஃபைலை அப்லோட் செய்தல்
    ...
    # ஃபைலை பப்ளிக் வியூ செய்ய பெர்மிஷன் வழங்குதல்
    file_id = file.get('id')
    ...
    # பப்ளிக் இமேஜ் லிங்கை திருப்புதல்
    return file.get('webContentLink') # அல்லது webViewLink
